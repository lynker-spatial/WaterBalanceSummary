...
ModelCorrMap <- function(x = model_correlation_by_divide, y = ModelScatterPlot){


  model_correlation_by_divide <- ModelScatterPlot <- NewDivides_correlation <- Divide_toid <- type <- ds_id <- areasqkm <- id <- lengthkm <- tot_drainage_areasqkm <- has_flowline <- pos_neg <- geom <- fill_var <- All_Sites_sf <- NewDivides <- correlation <- Divide <- toid <- NULL

  #Join with input data
  corr_divides <- left_join(NewDivides, x, by = "Divide")
  ...
  corr_divides <- corr_divides %>%
    ...
    dplyr::select(Divide, toid, type, ds_id, areasqkm, id, lengthkm, tot_drainage_areasqkm, has_flowline, correlation, pos_neg, geom) %>%
  ...
  assign("corr_divides_centroids", corr_divides_centroids, envir = WaterBalanceSummaryEnv)
  ...
  corr_map <- ggplot()+
    ...
    geom_label_repel(data = filter(corr_divides_centroids, !is.na(correlation)),
                     aes(geometry = geom, label = Divide),
