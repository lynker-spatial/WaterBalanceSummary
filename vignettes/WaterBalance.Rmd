...
that data back into the primary dataset for analysis. Assign your ouput
to [model name]\_delta_str
...
![](images/ExampleSeasonPlot.png) \### **Wide Data()** We need to make
the data wide before we can balance it and use it to create our water
balance plot. This can be done with the widen_model_data() function

```{r}
#cabcm_data_wide <- widen_model_data(cabcm_delta_str)
```

use cabcm_delta_str as the x value and feed the output right into
balance_data() below
